...
function renderDefineFees() {
    ...
    const stripeConfig = AppState.paymentSettings ? AppState.paymentSettings.stripe : { configured: false };

    return `
        <div class="fees-management-wrapper">
            <!-- Top Section: Define & Settings -->
            <div class="fees-top-grid">
                <!-- Define Fees Card -->
                <div class="premium-card">
                    <div class="card-gradient-header">
                        <div class="header-title-group">
                            <i class="fas fa-coins"></i>
                            <h3>Define Class Fees Structure</h3>
                        </div>
                    </div>
                        ...
                        <form id="define-fees-form">
                            ...
                            <div class="form-row">
                                <div class="form-group-custom">
                                    <label>Target Class</label>
                                    <select id="fees-class-id" class="modern-select" required>
                                ...
                                <div class="form-group-custom">
                                    ...
                                    <input type="number" id="tuition-fee" class="modern-input" placeholder="e.g. 50000" required>
                                </div>
                                <div class="form-group-custom">
                                    <label>Exam Fee (INR ₹)</label>
                                    <input type="number" id="exam-fee" class="modern-input" placeholder="e.g. 5000" required>
                                </div>
                                <div class="form-group-custom">
                                    <button type="button" id="btn-save-fees" class="btn-premium" onclick="saveClassFees()">
                ...
                <!-- Stripe Config Card -->
                <div class="premium-card">
                    <div class="card-gradient-header" style="background: #ffffff; color: #1e293b; border-bottom: 1px solid #f1f5f9;">
                        <div class="header-title-group">
                            <i class="fab fa-stripe-s" style="color: #635bff;"></i>
                            <h3 style="color: #1e293b;">Stripe Status</h3>
                        </div>
                    </div>
                    <div class="card-content-p30">
                        <div class="paypal-settings-card">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <span class="paypal-status-badge ${stripeConfig.configured ? 'active' : 'inactive'}">
                                    <i class="fas ${stripeConfig.configured ? 'fa-check-circle' : 'fa-times-circle'}"></i>
                                    ${stripeConfig.configured ? 'Connected' : 'Not Linked'}
                                </span>
                            </div>
                            <div class="paypal-config-info">
                                <div class="config-item">
                                    <span class="config-label">Publishable Key</span>
                                    <span class="config-value" style="font-size: 10px;">${stripeConfig.publishableKey || 'N/A'}</span>
                                </div>
                                <div class="config-item">
                                    <span class="config-label">Currency</span>
                                    <span class="config-value">${AppState.paymentSettings?.currency || 'INR'}</span>
                                </div>
                            </div>
                        </div>
                        <p style="margin-top: 15px; font-size: 11px; color: #94a3b8; text-align: center;">
                            Stripe credentials are loaded from the server's environment configuration.
                        </p>
                    </div>
                </div>
            </div>

            <!-- Bottom Section: Current Table -->
            <div class="premium-card">
                <div class="card-gradient-header" style="background: #f8fafc; color: #1e293b; border-bottom: 1px solid #e2e8f0;">
                    <div class="header-title-group">
                        <i class="fas fa-list-ul" style="color: #3b82f6;"></i>
                        <h3 style="color: #1e293b;">Current Fee Structures</h3>
                    </div>
                    <button class="btn-icon-alt" onclick="loadDefinedFees()" title="Refresh List">
                        <i class="fas fa-sync-alt"></i>
                    </button>
                </div>
                
                <div class="card-content-p30">
                    <div class="responsive-table-premium">
                        <table class="fees-table-premium">
                            <thead>
                                <tr>
                                    <th>Class</th>
                                    <th>Annual Fee</th>
                            ...
                            <tbody id="defined-fees-tbody">
                                <tr><td colspan="6" class="text-center py-5">
                                    <i class="fas fa-spinner fa-spin mr-2"></i> Syncing records...
                                </td></tr>
                            </tbody>
                        </table>
                    </div>
...
async function loadPaymentSettings() {
    try {
        ...
        if (response.ok) {
            const result = await response.json();
            if (result.success) {
                AppState.paymentSettings = result.data;
                // Re-render only if we are on the define-fees page to update the status card
                const currentContent = document.querySelector('.fees-management-wrapper');
                if (currentContent) {
                    const content = document.getElementById('admin-content') || document.getElementById('dashboard-content');
                    if (content) content.innerHTML = renderDefineFees();
                    // After re-render, we need to re-initialize listeners
                    await initializeDefineFees();
                }
            }
        }
...
async function saveClassFees() {
    ...
    try {
        ...
        if (result.success) {
            // Show toast or success indicator (using simple alert for now)
    ...
    try {
        ...
        if (result.success) {
            if (!result.data || result.data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" class="text-center py-5 text-muted">No fee records found.</td></tr>';
                return;
            }
            ...
            tbody.innerHTML = result.data
                ...
                .map(fee => {
                    ...
                    const className = classData.class || 'N/A';

                    return `
                    <tr>
                        <td><span style="font-size: 16px;">Class ${className}</span></td>
                        <td><span class="badge-fees">₹ ${fee.tuitionFee.toLocaleString()}</span></td>
                        <td><span class="badge-fees">₹ ${fee.examFee.toLocaleString()}</span></td>
                        <td><span class="badge-total">₹ ${fee.totalFee.toLocaleString()}</span></td>
                        <td><span style="color: #64748b; font-size: 12px;">${new Date(fee.updatedAt).toLocaleDateString()}</span></td>
                        <td style="text-align: right;">
                            <button class="btn-icon-alt" title="Edit Structure" onclick="editFee('${className}', ${fee.tuitionFee}, ${fee.examFee})">
                                <i class="fas fa-edit"></i>
                            </button>
                        </td>
                    </tr>
        ...
        tbody.innerHTML = '<tr><td colspan="6" class="text-center py-5 text-danger">Error syncing records.</td></tr>';
    }
}

function editFee(className, tuition, exam) {
    const classSelect = document.getElementById('fees-class-id');
    const tuitionInput = document.getElementById('tuition-fee');
    const examInput = document.getElementById('exam-fee');
    ...
    const formCard = document.querySelector('.premium-card');
...
async function loadPaymentClasses() {
    try {
        ...
        const result = await response.json();
        if (result.success && result.data) {
            // Extract unique classes with both ID and Name
            const classesMap = new Map();
            result.data.forEach(s => {
                if (s.class_id && s.class) {
                    classesMap.set(s.class_id.toString(), s.class);
                }
            });

            const classSelect = document.getElementById('payment-class-filter');
            if (classSelect) {
                // Sort classes by name/number
                const sortedEntries = [...classesMap.entries()].sort((a, b) => a[1] - b[1]);

                classSelect.innerHTML = '<option value="">All Classes</option>' +
                    sortedEntries.map(([id, name]) => `<option value="${id}">${name}</option>`).join('');
            }
        }
    } catch (error) {
        console.error('Error loading classes:', error);
    ...
    tbody.innerHTML = students.map(student => {
        ...
        return `
            <tr>
                ...
                `<span class="status-badge status-pending" style="padding: 6px 12px; font-size: 11px;">Awaiting Online Payment</span>`
            }
                </td>
            </tr>
        `;
