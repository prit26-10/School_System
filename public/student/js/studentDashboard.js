...
// ────────────────────────────────────────────────
// FEE PAYMENTS & PAYPAL INTEGRATION
// ────────────────────────────────────────────────
...
async function loadStudentFees() {
    ...
    container.innerHTML = `
        <div style="text-align: center; padding: 80px 20px;">
            <i class="fas fa-spinner fa-spin" style="font-size: 32px; color: #2563eb; margin-bottom: 20px;"></i>
            <p style="color: #64748b; font-weight: 500;">Securing your financial records...</p>
        </div>
    `;
    ...
    try {
        ...
        const result = await response.json();
        if (result.success) {
            renderStudentFees(result.data, result.isPaid, result.paymentDetails);
        } else {
            container.innerHTML = `
                <div style="text-align: center; padding: 80px 20px;">
                    <div style="width: 80px; height: 80px; background: #fff1f2; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px;">
                        <i class="fas fa-exclamation-triangle" style="font-size: 32px; color: #e11d48;"></i>
                    </div>
                    <h3 style="color: #1e293b; font-weight: 700;">Records Not Found</h3>
                    <p style="color: #64748b;">${result.message || 'Fee structure has not been defined for your class yet.'}</p>
                </div>
            `;
        }
    } catch (err) {
        console.error('Error loading fees:', err);
        container.innerHTML = `<div style="text-align: center; padding: 60px; color: #e11d48;">Network error occurred while syncing records.</div>`;
    }
}

function renderStudentFees(feeData, isPaid, paymentDetails) {
    const container = document.getElementById('student-fees-container');
    if (!container) return;

    const totalStr = (feeData.totalFee || 0).toLocaleString('en-US', { style: 'currency', currency: 'USD' });
    const tuitionStr = (feeData.tuitionFee || 0).toLocaleString('en-US', { style: 'currency', currency: 'USD' });
    const examStr = (feeData.examFee || 0).toLocaleString('en-US', { style: 'currency', currency: 'USD' });
    ...
    if (isPaid) {
        statusHtml = `
            <div style="background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; padding: 16px; margin-bottom: 24px; display: flex; align-items: center; gap: 16px;">
                <div style="width: 44px; height: 44px; background: #16a34a; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 20px;">
                    <i class="fas fa-check"></i>
                </div>
                <div>
                    <h4 style="margin: 0; color: #166534; font-size: 15px; font-weight: 700;">Payment Successful</h4>
                    <p style="margin: 4px 0 0; color: #15803d; font-size: 13px;">Your fee record is up-to-date. ID: ${paymentDetails.paypalCaptureId || paymentDetails.paymentId}</p>
                </div>
            </div>
        `;
        actionHtml = `
            <div style="text-align: center; padding: 20px; background: #f8fafc; border-radius: 12px; border: 1px dashed #e2e8f0;">
                <i class="fas fa-file-invoice-dollar" style="font-size: 24px; color: #94a3b8; margin-bottom: 8px;"></i>
                <p style="margin: 0; color: #64748b; font-size: 14px; font-weight: 600;">Invoice generated on ${new Date(paymentDetails.updatedAt).toLocaleDateString()}</p>
            </div>
        `;
    } else {
        statusHtml = `
            <div style="background: #fffbeb; border: 1px solid #fef3c7; border-radius: 12px; padding: 16px; margin-bottom: 24px; display: flex; align-items: center; gap: 16px;">
                <div style="width: 44px; height: 44px; background: #d97706; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 20px;">
                    <i class="fas fa-hourglass-half"></i>
                </div>
                <div>
                    <h4 style="margin: 0; color: #92400e; font-size: 15px; font-weight: 700;">Payment Pending</h4>
                    <p style="margin: 4px 0 0; color: #b45309; font-size: 13px;">Please settle your outstanding balance to avoid late charges.</p>
                </div>
            </div>
        `;
        actionHtml = `
            <div id="paypal-button-container" style="margin-top: 20px;">
                ...
                <div style="text-align: center; padding: 10px;">
                    <i class="fas fa-spinner fa-spin"></i> Initializing Secure Gateway...
                </div>
            </div>
        `;
    }
    ...
    container.innerHTML = `
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; align-items: start;">
            <!-- Left: Summaries -->
            <div style="background: #f8fafc; border-radius: 20px; padding: 30px; border: 1px solid #f1f5f9;">
                <h4 style="margin: 0 0 20px 0; font-size: 14px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 1px;">Session Breakdown</h4>
                
                <div style="display: flex; flex-direction: column; gap: 18px;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="color: #475569; font-weight: 500;">Tuition Fee</span>
                        <span style="color: #1e293b; font-weight: 700;">${tuitionStr}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="color: #475569; font-weight: 500;">Examination Fee</span>
                        <span style="color: #1e293b; font-weight: 700;">${examStr}</span>
                    </div>
                    <div style="height: 1px; background: #e2e8f0; margin: 5px 0;"></div>
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="color: #1e293b; font-weight: 800; font-size: 16px;">Total Payable</span>
                        <span style="color: #2563eb; font-weight: 900; font-size: 20px;">${totalStr}</span>
                    </div>
                </div>
            </div>

            <!-- Right: Status and Pay -->
            <div>
                ${statusHtml}
                ${actionHtml}
            </div>
    ...
    if (!isPaid) {
        initPayPalFlow(feeData.totalFee, feeData.classId?._id || feeData.classId);
    }
}

async function initPayPalFlow(amount, classId) {
    try {
        const token = localStorage.getItem('token');
        const configRes = await fetch('/api/fees/settings', {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        if (!configResult.success || !configResult.data.paypal.configured) {
            ...
            if (btnContainer) {
                btnContainer.innerHTML = `
                    <div style="padding: 15px; background: #fef2f2; color: #991b1b; border-radius: 8px; font-size: 13px; font-weight: 600;">
                        <i class="fas fa-exclamation-circle"></i> PayPal gateway is not configured by administration.
                    </div>
                `;
            }
            return;
        ...
        if (!window.paypal) {
            ...
            script.onload = () => setupPayPalButtons(amount, classId);
            document.head.appendChild(script);
        } else {
            setupPayPalButtons(amount, classId);
        }
    } catch (err) {
        ...
        createOrder: async function(data, actions) {
            try {
                ...
                if (result.success) {
                    ...
                    alert('Failed to initiate PayPal order: ' + result.message);
                }
            } catch (err) {
                console.error(err);
        ...
        onApprove: async function(data, actions) {
            try {
                ...
                if (result.success) {
                    ...
                    alert('Transaction failed to capture. Please contact admin.');
                }
            } catch (err) {
                console.error(err);
...
async function checkPaymentStatus() {
    try {
        ...
        if (response.status === 403 || response.status === 401) {
            console.warn('Fee check unauthorized. Showing default lockout modal.');
            showLockoutModal({ totalFee: 0, unauthorized: true });
            return;
        }
        ...
        if (result.success) {
            // Unlock if:
            // 1. Student has already paid
            // 2. Fee structure is not defined (totalFee 0)
            if (result.isPaid || !result.data || (result.data && result.data.totalFee <= 0)) {
                unlockDashboard();
            } else {
                // Otherwise show the prompt
                showLockoutModal(result.data);
            }
        } else {
            // If the check fails (e.g. 404 or structure error), keep it locked for safety
            console.warn('Fee check failed:', result.message);
            showLockoutModal({ totalFee: 0, unauthorized: true, message: result.message });
        }
    } catch (err) {
...
function showLockoutModal(feeData) {
    const overlay = document.getElementById('fee-lockout-overlay');
    const modal = document.getElementById('fee-lockout-modal');
    const amountSpan = document.getElementById('lockout-fee-amount');
    ...
    if (feeData.unauthorized) {
        ...
        if (amountSpan) {
            const amount = feeData.totalFee || 0;
            amountSpan.textContent = amount.toLocaleString('en-IN', { style: 'currency', currency: 'INR' });
        }
        if (description) {
            description.textContent = 'You have pending fees. Click below to pay securely via Stripe.';
        }
    }

    // Initialize Stripe button in the modal
    initStripeLocked(feeData.totalFee || 0, feeData.classId?._id || feeData.classId);
}

async function initStripeLocked(amount, classId) {
    try {
        const token = localStorage.getItem('token');
        const configRes = await fetch('/api/fees/settings', {
            headers: { 'Authorization': `Bearer ${token}` }
        ...
        if (!configResult.success || !configResult.data.stripe.configured) {
            btnContainer.innerHTML = `
                <div style="padding: 15px; background: #fef2f2; color: #991b1b; border-radius: 8px; font-size: 13px; font-weight: 600;">
                    <i class="fas fa-exclamation-circle"></i> Stripe gateway is not configured. Please contact school admin.
                </div>
            `;
            return;
        }
        ...
        document.getElementById('stripe-checkout-button').addEventListener('click', async () => {
            ...
            try {
                const res = await fetch('/api/fees/stripe/session', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
                ...
                if (result.success && result.url) {
                    ...
                    alert('Error: ' + (result.message || 'Failed to start payment'));
                    btn.disabled = false;
...
async function verifyStripePayment(sessionId) {
    try {
        const token = localStorage.getItem('token');
        const res = await fetch('/api/fees/stripe/verify', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
        ...
        const result = await res.json();
        if (result.success) {
            alert('Access Granted! Your dashboard is now fully unlocked.');
            window.location.href = '/student/html/studentDashboard.html'; // Clear URL params
        }
    } catch (e) {
...
function unlockDashboard() {
    ...
    if (mainContent) mainContent.classList.remove('dashboard-locked');
    ...
    if (activeNav) {
        ...
        if (typeof loadPageData === 'function') loadPageData(pageId);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (urlParams.get('payment') === 'success' && urlParams.get('session_id')) {
        verifyStripePayment(urlParams.get('session_id'));
    } else {
        checkPaymentStatus();
    }
});
