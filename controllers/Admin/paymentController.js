  ...
  try {
    ...
    if (class_id) {
      filteredStudents = studentsWithClassInfo.filter(s => 
        ...
        (s.class && s.class.toString() === class_id)
      );
    }
    ...
    // Create fee payment map (Stripe transactions)
    const feePaymentMap = new Map();
    feePayments.forEach(fp => {
      const sid = fp.studentId.toString();
      if (!feePaymentMap.has(sid)) {
        feePaymentMap.set(sid, []);
      }
      feePaymentMap.get(sid).push(fp);
    });
    ...
    for (const student of filteredStudents) {
      ...
      // Cross-check with FeePayment (Stripe) records
      const stripePayments = feePaymentMap.get(student._id.toString()) || [];
      const totalStripePaid = stripePayments.reduce((sum, fp) => sum + fp.amount, 0);
      
      // If Stripe payments found, update the display status/amount
      // This ensures that even if StudentPayment hasn't been synced yet, 
      // the admin sees the real-time status from Stripe.
      if (totalStripePaid > 0) {
        paidAmount = Math.max(paidAmount, totalStripePaid);
...
exports.debugData = async (req, res) => {
  try {
    ...
    const feePayments = await FeePayment.find({}).sort({ createdAt: -1 }).limit(50);
    const studentPayments = await StudentPayment.find({}).limit(20);
...
exports.getAllStudentPayments = async (req, res) => {
  try {
    ...
    // Also fetch FeePayment records (Stripe)
    const feePayments = await FeePayment.find({ 
      studentId: { $in: studentIds },
      status: "completed"
    });
    ...
    for (const student of studentsWithClassInfo) {
      ...
      // If no payment record exists, check for Stripe payments
      if (!payment) {
        const stripePayments = feePaymentMap.get(student._id.toString()) || [];
        if (stripePayments.length === 0) {
          continue; // Skip if no payment found at all
        }
        
        // Use class fees for summary
        let clsFees = 0;
        if (student.classInfo) {
          const cf = await ClassFees.findOne({ classId: student.classInfo._id });
          if (cf) clsFees = cf.totalFee;
        }

        const totalStripe = stripePayments.reduce((sum, fp) => sum + fp.amount, 0);
        
        result.push({
          _id: stripePayments[0]._id, // Use one of the Stripe record IDs
          studentName: student.name,
          studentEmail: student.email,
          class: student.classInfo?.class || student.class || 'N/A',
          totalFees: clsFees,
          paidAmount: totalStripe,
          dueAmount: Math.max(0, clsFees - totalStripe),
          paymentStatus: (totalStripe >= clsFees && clsFees > 0) ? "paid" : "pending",
          dueDate: new Date(),
          lastPaymentDate: stripePayments[stripePayments.length - 1].updatedAt,
          paymentHistory: stripePayments.map(fp => ({
            amount: fp.amount,
            paymentDate: fp.updatedAt,
            paymentMethod: 'stripe',
            receiptNumber: fp.transactionId || fp.paymentId,
